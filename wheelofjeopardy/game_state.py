...
class GameState(object):
    ...
    def set_category(self, category):
        ...
        self._process_a_question()

    def _process_a_question(self):
        events = self.events
        board_state = self.board
        if self.board.no_q_in_round(self.current_round):
            if (self.current_round == 1):
                self.current_round += 1
                events.broadcast('game_state.round_did_end')
            elif (self.current_round == 2):
                events.broadcast('game_state.game_did_end')
        elif board_state.no_q_in_category(self.current_round,
                                          self.current_category):
            events.broadcast('game_state.prompt_no_remaining_q_spin_again')
        else:
